...
import { ArrowLeft, Minus, Plus, Trash2, X, Loader2, AlertTriangle } from "lucide-react"
...
export default function EditProductPage({ params }: { params: { id: string } }) {
  const { getProduct, updateProduct, isLoading: areProductsLoading } = useProducts()
    ...
    if (!areProductsLoading) {
      ...
      if (foundProduct) {
        setFormData({
          ...
          categoriaId: foundProduct.categoriaId,
          categoriaNome: foundProduct.categoriaNome,
          material: foundProduct.material,
          code: foundProduct.code,
          currentPrice: foundProduct.currentPrice,
          originalPrice: foundProduct.originalPrice,
          onSale: foundProduct.onSale,
  ...
  }, [params.id, getProduct, router, toast, areProductsLoading]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Recalcula o total baseado nas variantes atuais antes de enviar
      const calculatedGeneralStock = sizes.reduce((acc, curr) => acc + curr.stock, 0);

      const payload = {
        ...formData,
        generalStock: calculatedGeneralStock, 
        details,
        sizes 
      }
  ...
  const addSize = () => { 
    if (newSize.size.trim() && newSize.stock >= 0) { 
      setSizes([...sizes, { id: `temp-${Date.now()}`, size: newSize.size.trim(), stock: newSize.stock }]); 
      setNewSize({ size: "", stock: 0 }) 
    } 
  }
  const removeSize = (id: string) => { setSizes(sizes.filter((s) => s.id !== id)) }
  const updateSizeStock = (id: string, stock: number) => { 
    setSizes(sizes.map((s) => (s.id === id ? { ...s, stock: Math.max(0, stock) } : s))) 
  }

  // Display calculado para feedback visual
  ...
  return (
    <AdminRouteGuard>
      <div className="w-full max-w-3xl mx-auto pb-10">
        <div className="mb-6"><Link href="/perfil/admin/produtos"><Button variant="ghost"><ArrowLeft className="mr-2 h-4 w-4"/> Voltar</Button></Link></div>
        <h1 className="mb-8 text-3xl font-bold text-center">Editar Produto</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
               ...
               <div className="grid gap-4 md:grid-cols-2">
                 ...
                 <div className="space-y-2"><Label>Material</Label><Input value={formData.material} onChange={(e) => setFormData({ ...formData, material: e.target.value })}/></div>
               </div>
               <div className="grid gap-4 md:grid-cols-2">
                 ...
                 {/* CAMPO DE INPUT REMOVIDO AQUI TAMBÉM */}
                 <div className="space-y-2">
          ...
          <Card>
             ...
             <CardContent className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2"><Label>Preço Atual</Label><Input type="number" value={formData.currentPrice} onChange={(e) => setFormData({ ...formData, currentPrice: Number(e.target.value) })}/></div>
          ...
          <Card className="border-orange-200 bg-orange-50/30">
            <CardHeader>
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1"><AlertTriangle className="h-3 w-3 text-orange-500" />Edite quantidades aqui apenas se necessário (sobrescreve).</p>
            </CardHeader>
            <CardContent className="space-y-4">
              {sizes.map((size) => (
                <div key={size.id} className="flex items-center gap-2 rounded-lg border bg-card p-3">
                  ...
                  <div className="flex items-center gap-1">
                    <Button type="button" variant="outline" size="icon" onClick={() => updateSizeStock(size.id, size.stock - 1)} className="h-8 w-8"><Minus className="h-4 w-4" /></Button>
                    <span className="w-12 text-center font-medium">{size.stock}</span>
                    <Button type="button" variant="outline" size="icon" onClick={() => updateSizeStock(size.id, size.stock + 1)} className="h-8 w-8"><Plus className="h-4 w-4" /></Button>
                  </div>
                  <Button type="button" variant="ghost" size="icon" onClick={() => removeSize(size.id)} className="h-8 w-8 text-destructive"><Trash2 className="h-4 w-4" /></Button>
                </div>
              ))}
          ...
          <div className="flex gap-4">
            <Button type="submit" size="lg" className="flex-1" disabled={isSubmitting}>{isSubmitting ? <Loader2 className="animate-spin" /> : "Salvar"}</Button>
